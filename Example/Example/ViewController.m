...
- (void)viewDidLoad {
  ...
  // Initialize the ATTNEventTracker. This must be done before the
  // ATTNEventTracker can be used to send any events.
  [ATTNEventTracker setupWithSdk:sdk];

  // Register the current user with the Attentive SDK by calling the `identify`
  // method. Each identifier is optional, but the more identifiers you provide
  // the better the Attentive SDK will function.
  _userIdentifiers = @{
...
- (IBAction)sendIdentifiersButtonPress:(id)sender {
  // Sends the identifiers to Attentive. This should be done whenever a new
...
- (void)setupForUITests {
  // Override the hard-coded domain & mode with the values from the environment
  // variables
  NSString *envDomain = [[[NSProcessInfo processInfo] environment]
      objectForKey:@"COM_ATTENTIVE_EXAMPLE_DOMAIN"];
  NSString *envMode = [[[NSProcessInfo processInfo] environment]
      objectForKey:@"COM_ATTENTIVE_EXAMPLE_MODE"];
  ...
  // Reset the standard user defaults - this must be done from within the app to
  // avoid race conditions
  NSString *isUITest = [[[NSProcessInfo processInfo] environment]
      objectForKey:@"COM_ATTENTIVE_EXAMPLE_IS_UI_TEST"];
