...
- (void)testLoadCreative_clickClose_closesCreative {
  ...
  // Close the creative
  XCTAssertTrue(
      [app.webViews.buttons[@"close"] waitForExistenceWithTimeout:5.0]);
  [app.webViews.buttons[@"close"] tap];

  // Assert that the creative is closed
  XCTAssertTrue(
      [app.buttons[@"Push me for Creative!"] waitForExistenceWithTimeout:5.0]);
  XCTAssertEqual(app.buttons[@"Push me for Creative!"].isHittable, true);
}

- (void)
    testLoadCreative_fillOutFormAndSubmit_launchesSmsAppWithPrePopulatedText {
  [self launchAppWithMode:@"production"];
  [app.buttons[@"Push me for Creative!"] tap];

  // Fill in the email
  XCTAssertTrue([app.webViews.textFields[@"Email Address"]
  ...
  XCTAssertTrue(
      [app.buttons[@"GET 10% OFF NOW when you sign up for email and texts"]
          waitForExistenceWithTimeout:5.0]);
  ...
  NSString *envHost = [[[NSProcessInfo processInfo] environment]
      objectForKey:@"COM_ATTENTIVE_EXAMPLE_HOST"];
  if ([envHost isEqualToString:@"local"]) {
    XCUIApplication *smsApp = [[XCUIApplication alloc]
        initWithBundleIdentifier:@"com.apple.MobileSMS"];
    XCTAssertTrue(
        [smsApp.textFields[@"Message"] waitForExistenceWithTimeout:5.0]);
    XCTAssertTrue([smsApp.textFields[@"Message"].value
        containsString:@"Send this text to subscribe to recurring automated "
                       @"personalized marketing alerts (e.g. cart reminders) "
                       @"from Attentive Mobile Apps Test"]);
  }
}
...
- (void)testLoadCreative_clickPrivacyLink_opensPrivacyPageInWebApp {
  ...
  // Click privacy link
  XCTAssertTrue(
      [app.webViews.links[@"Privacy"] waitForExistenceWithTimeout:5.0]);
  [app.webViews.links[@"Privacy"] tap];

  // Verify that the privacy page is visible in the external browser
  XCUIApplication *safariApp = [[XCUIApplication alloc]
      initWithBundleIdentifier:@"com.apple.mobilesafari"];
  XCTAssertTrue([safariApp.staticTexts[@"Privacy Policy"]
      waitForExistenceWithTimeout:5.0]);
}
...
- (void)testLoadCreative_inDebugMode_showsDebugMessage {
  ...
  XCTAssertTrue(
      [app.staticTexts[@"Debug output JSON:"] waitForExistenceWithTimeout:5.0]);
}
...
+ (void)resetUserDefaults {
  ...
  [[NSUserDefaults standardUserDefaults]
      removePersistentDomainForName:@"com.attentive.Example"];
}
