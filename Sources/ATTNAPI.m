...
// A single event can create multiple requests. The EventRequest class
...
- (instancetype)initWithMetadata:(NSMutableDictionary *)metadata
           eventNameAbbreviation:(NSString *)abbreviation;
...
- (NSURLSession *)buildUrlSession {
  NSURLSessionConfiguration *configWithUserAgent =
      [NSURLSessionConfiguration defaultSessionConfiguration];
  NSDictionary *additionalHeadersWithUserAgent =
      @{@"User-Agent" : [ATTNUserAgentBuilder buildUserAgent]};
...
// TODO: When we add the other events, the USER_IDENTIFIER_COLLECTED event code
// will be wrapped into the generic event code
- (void)sendUserIdentity:(ATTNUserIdentity *)userIdentity {
  [self sendUserIdentity:userIdentity callback:nil];
}

- (void)sendUserIdentity:(ATTNUserIdentity *)userIdentity
                callback:(ATTNAPICallback)callback {
  ...
  [self getGeoAdjustedDomain:_domain
           completionHandler:^(NSString *geoAdjustedDomain, NSError *error) {
             ...
             [self sendUserIdentityInternal:userIdentity
                                     domain:geoAdjustedDomain
                                   callback:callback];
           }];
}

- (void)sendEvent:(id<ATTNEvent>)event
...
- (void)sendEventInternal:(id<ATTNEvent>)event
             userIdentity:(ATTNUserIdentity *)userIdentity
  ...
  for (EventRequest *request in requests) {
    [self sendEventInternalForRequest:request
                         userIdentity:userIdentity
                               domain:domain
                             callback:callback];
  }
...
- (void)sendEventInternalForRequest:(EventRequest *)request
                       userIdentity:(ATTNUserIdentity *)userIdentity
                             domain:(NSString *)domain
                           callback:(ATTNAPICallback)callback {
  NSURL *url = [self constructEventUrlComponentsForEventRequest:request
                                                   userIdentity:userIdentity
                                                         domain:domain]
                   .URL;
  ...
  NSURLSessionDataTask *task = [_urlSession
      dataTaskWithRequest:urlRequest
        completionHandler:^void(NSData *_Nullable data,
          ...
          if (error) {
            message = [NSString
                stringWithFormat:@"Error sending for event '%@'. Error: '%@'",
                                 request.eventNameAbbreviation,
                                 [error description]];
          } else {
            // The response is an HTTP response because the URL had an HTTPS
            // scheme
            NSHTTPURLResponse *httpResponse = (NSHTTPURLResponse *)response;
            if ([httpResponse statusCode] != 200) {
              message = [NSString
                  stringWithFormat:
                      @"Error sending the event. Incorrect status code: '%ld'",
                      (long)[httpResponse statusCode]];
            } else {
              message = [NSString
                  stringWithFormat:@"Successfully sent event of type '%@'",
                                   request.eventNameAbbreviation];
            }
          }
...
// A single event can create multiple requests to our servers. This method
// converts an event to multiple request objects.
  ...
  if ([event isKindOfClass:[ATTNPurchaseEvent class]]) {
    ...
    for (ATTNItem *item in purchase.items) {
      ...
      if (purchase.cart != nil) {
        ...
        [metadata addEntryIfNotNil:@"cartCoupon"
                             value:purchase.cart.cartCoupon];
      }

      [eventRequests addObject:[[EventRequest alloc]
                                        initWithMetadata:metadata
                                   eventNameAbbreviation:EVENT_TYPE_PURCHASE]];
    ...
    NSMutableDictionary *orderConfirmedMetadata =
    ...
    NSMutableArray<NSMutableDictionary *> *products =
    ...
    orderConfirmedMetadata[@"products"] = [self convertObjectToJson:products
                                                       defaultValue:@"[]"];
    [eventRequests
        addObject:[[EventRequest alloc]
                           initWithMetadata:orderConfirmedMetadata
                      eventNameAbbreviation:EVENT_TYPE_ORDER_CONFIRMED]];
    ...
    for (ATTNItem *item in addToCart.items) {
      ...
      [eventRequests
          addObject:[[EventRequest alloc]
                             initWithMetadata:metadata
                        eventNameAbbreviation:EVENT_TYPE_ADD_TO_CART]];
    }
    ...
    for (ATTNItem *item in productView.items) {
      ...
      [eventRequests
          addObject:[[EventRequest alloc]
                             initWithMetadata:metadata
                        eventNameAbbreviation:EVENT_TYPE_PRODUCT_VIEW]];
    }
    ...
    [eventRequests
        addObject:[[EventRequest alloc]
                           initWithMetadata:[[NSMutableDictionary alloc] init]
                      eventNameAbbreviation:EVENT_TYPE_INFO]];
    return eventRequests;
  } else if ([event isKindOfClass:[ATTNCustomEvent class]]) {
    ATTNCustomEvent *customEvent = (ATTNCustomEvent *)event;

    NSMutableDictionary *customEventMetadata =
        [[NSMutableDictionary alloc] init];
    customEventMetadata[@"type"] = customEvent.type;
    customEventMetadata[@"properties"] =
        [self convertObjectToJson:customEvent.properties defaultValue:@"{}"];

    [eventRequests
        addObject:[[EventRequest alloc]
                           initWithMetadata:customEventMetadata
                      eventNameAbbreviation:EVENT_TYPE_CUSTOM_EVENT]];
    return eventRequests;
  } else {
    NSException *e = [NSException
        exceptionWithName:@"UnknownEventException"
                   reason:[NSString stringWithFormat:@"Unknown Event type: %@",
                                                     [event class]]
                 userInfo:nil];
    @throw e;
  }
...
- (void)addProductDataToDictionary:(ATTNItem *)item
...
- (void)getGeoAdjustedDomain:(NSString *)domain
  ...
  NSLog(
      @"%@",
      [NSString
          stringWithFormat:@"Getting the geoAdjustedDomain for domain '%@'...",
                           domain]);
  ...
  NSURLSessionDataTask *task = [_urlSession
      dataTaskWithRequest:request
        completionHandler:^void(NSData *_Nullable data,
                                ...
                                NSError *_Nullable error) {
          if (error) {
            NSLog(@"Error getting the geo-adjusted domain. Error: '%@'",
                  [error description]);
          ...
          // scheme
          NSHTTPURLResponse *httpResponse = (NSHTTPURLResponse *)response;
          // The body/data only contains the tag data if the endpoint returns a
          // 200
          if ([httpResponse statusCode] != 200) {
            NSLog(@"Error getting the geo-adjusted domain. Incorrect status "
                  @"code: '%ld'",
                  (long)[httpResponse statusCode]);
            completionHandler(
                nil, [NSError errorWithDomain:@"com.attentive.API"
          ...
          NSString *geoAdjustedDomain =
              [ATTNAPI extractDomainFromTag:dataString];

          if (!geoAdjustedDomain) {
            NSError *error =
                [NSError errorWithDomain:@"com.attentive.API"
                                    code:NSURLErrorBadServerResponse
                                userInfo:nil];
            completionHandler(nil, error);
            return;
...
- (void)sendUserIdentityInternal:(ATTNUserIdentity *)userIdentity
                          domain:(NSString *)domain
                        callback:(ATTNAPICallback)callback {
  ...
  NSURLSessionDataTask *task = [_urlSession
      dataTaskWithRequest:request
        completionHandler:^void(NSData *_Nullable data,
          ...
          if (error) {
            message = [NSString
                stringWithFormat:@"Error sending user identity. Error: '%@'",
                                 [error description]];
          } else {
            // The response is an HTTP response because the URL had an HTTPS
            // scheme
...
- (NSURLComponents *)
    constructEventUrlComponentsForEventRequest:(EventRequest *)eventRequest
                                  userIdentity:(ATTNUserIdentity *)userIdentity
                                        domain:(NSString *)domain {
  NSURLComponents *urlComponents = [[NSURLComponents alloc]
      initWithString:@"https://events.attentivemobile.com/e"];

  NSMutableDictionary *queryParams = [self constructBaseQueryParams:userIdentity
                                                             domain:domain];
  ...
  queryParams[@"m"] = [self convertObjectToJson:combinedMetadata
                                   defaultValue:@"{}"];
...
- (NSMutableDictionary *)constructBaseQueryParams:
...
- (NSURLComponents *)constructUserIdentityUrl:(ATTNUserIdentity *)userIdentity
                                       domain:(NSString *)domain {
  NSURLComponents *urlComponents = [[NSURLComponents alloc]
      initWithString:@"https://events.attentivemobile.com/e"];
...
- (NSString *)buildExternalVendorIdsJson:(ATTNUserIdentity *)userIdentity {
  ...
  if (userIdentity.identifiers[IDENTIFIER_TYPE_CLIENT_USER_ID]) {
    [ids addObject:@{
      @"vendor" : EXTERNAL_VENDOR_TYPE_CLIENT_USER,
      @"id" : userIdentity.identifiers[IDENTIFIER_TYPE_CLIENT_USER_ID]
    }];
  }
  if (userIdentity.identifiers[IDENTIFIER_TYPE_KLAVIYO_ID]) {
    [ids addObject:@{
      @"vendor" : EXTERNAL_VENDOR_TYPE_KLAVIYO,
      @"id" : userIdentity.identifiers[IDENTIFIER_TYPE_KLAVIYO_ID]
    }];
  }
  if (userIdentity.identifiers[IDENTIFIER_TYPE_SHOPIFY_ID]) {
    [ids addObject:@{
      @"vendor" : EXTERNAL_VENDOR_TYPE_SHOPIFY,
      @"id" : userIdentity.identifiers[IDENTIFIER_TYPE_SHOPIFY_ID]
    }];
  }
  if (userIdentity.identifiers[IDENTIFIER_TYPE_CUSTOM_IDENTIFIERS]) {
    NSDictionary *customIdentifiers =
        userIdentity.identifiers[IDENTIFIER_TYPE_CUSTOM_IDENTIFIERS];
    for (id key in customIdentifiers) {
      [ids addObject:@{
        @"vendor" : EXTERNAL_VENDOR_TYPE_CUSTOM_USER,
        @"id" : customIdentifiers[key],
        @"name" : key
      }];
    }
  }
  ...
  NSError *error = nil;
  NSData *array = [NSJSONSerialization dataWithJSONObject:ids
                                                  options:0
                                                    error:&error];

  if (error) {
    NSLog(@"Could not serialize the external vendor ids. Returning an empty "
          @"array. Error: '%@'",
          [error description]);
    return @"[]";
  }
...
- (NSString *)convertObjectToJson:(id)object
  ...
  if (error) {
    NSLog(@"Could not serialize the object to JSON. Error: '%@'",
          [error description]);
    return defaultValue;
  }
...
+ (NSString *_Nullable)extractDomainFromTag:(NSString *)tag {
  ...
  NSRegularExpression *regex = [NSRegularExpression
      regularExpressionWithPattern:@"window.__attentive_domain='(.*?).attn.tv'"
                           options:0
                             error:&error];
  ...
  NSInteger matchesCount =
      [regex numberOfMatchesInString:tag
                             options:0
  ...
  if (matchesCount < 1) {
    ...
    NSLog(@"More than one match found for the Attentive domain regex. "
          @"Continuing with the first one...");
  }

  NSTextCheckingResult *regexResult =
      [regex firstMatchInString:tag
                        options:0
                          range:NSMakeRange(0, [tag length])];
  if (!regexResult) {
