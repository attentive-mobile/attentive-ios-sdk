...
- (void)identify:(NSDictionary *)userIdentifiers {
  if ([userIdentifiers isKindOfClass:[NSString class]]) {
    ...
    NSLog(
        @"WARNING: This way of calling identify is deprecated. Please pass in "
        @"userIdentifiers as an <NSDictionary *>. See SDK README for details.");
    [_userIdentity mergeIdentifiers:@{
      IDENTIFIER_TYPE_CLIENT_USER_ID : (NSString *)userIdentifiers
    }];
  } else if ([userIdentifiers isKindOfClass:[NSDictionary class]]) {
    [_userIdentity mergeIdentifiers:(NSDictionary *)userIdentifiers];
  } else {
    [NSException raise:@"Incorrect type for userIdentifiers"
                format:@"userIdentifiers should be of type <NSDictionary *>"];
  }
...
- (void)trigger:(UIView *)theView {
  ...
  if (@available(iOS 14, *)) {
    NSLog(@"The iOS version is new enough, continuing to show the Attentive "
          @"creative.");
  } else {
    NSLog(@"Not showing the Attentive creative because the iOS version is too "
          @"old.");
    return;
  }
  ...
  NSString *userScriptWithEventListener =
      @"window.addEventListener('message', (event) => {if (event.data && "
      @"event.data.__attentive && event.data.__attentive.action === 'CLOSE') "
      @"{window.webkit.messageHandlers.log.postMessage(event.data.__attentive."
      @"action);}}, false);";

  WKUserScript *wkUserScript = [[WKUserScript alloc]
        initWithSource:userScriptWithEventListener
         injectionTime:WKUserScriptInjectionTimeAtDocumentStart
      forMainFrameOnly:FALSE];
  [[wkWebViewConfiguration userContentController] addUserScript:wkUserScript];

  _webView = [[WKWebView alloc] initWithFrame:theView.frame
...
- (void)webView:(WKWebView *)webView
  ...
  if ([url.scheme isEqualToString:@"sms"]) {
    ...
    // If the targetFrame is nil then the link was defined to open in a new tab
    // (two examples of this are the Privacy and Terms links). In this case,
    // open the url in the phone's web browser instead of the webview.
