...
+ (nonnull NSString *)buildCompanyCreativeUrlForDomain:(NSString *)domain
  ...
  NSURLComponents *components = [[NSURLComponents alloc]
      initWithString:@"https://creatives.attn.tv/mobile-apps/index.html"];

  // Add query parameters
  NSMutableArray *queryItems = [[NSMutableArray alloc]
      initWithObjects:[[NSURLQueryItem alloc] initWithName:@"domain"
                                                     value:domain],
                      nil];
  ...
  if ([mode isEqualToString:@"debug"]) {
    [queryItems addObject:[[NSURLQueryItem alloc]
                              initWithName:mode
                                     value:@"matter-trip-grass-symbol"]];
  }

  if (userIdentity.visitorId != nil) {
    [queryItems
        addObject:[[NSURLQueryItem alloc] initWithName:@"vid"
                                                 value:userIdentity.visitorId]];
  } else {
  ...
  if (userIdentity.identifiers[IDENTIFIER_TYPE_CLIENT_USER_ID] != nil) {
    [queryItems
        addObject:[[NSURLQueryItem alloc]
                      initWithName:@"cuid"
                             value:userIdentity.identifiers
  ...
  if (userIdentity.identifiers[IDENTIFIER_TYPE_PHONE] != nil) {
    [queryItems
        addObject:[[NSURLQueryItem alloc]
                      initWithName:@"p"
                             value:userIdentity
                                       .identifiers[IDENTIFIER_TYPE_PHONE]]];
  }
  ...
  if (userIdentity.identifiers[IDENTIFIER_TYPE_EMAIL] != nil) {
    [queryItems
        addObject:[[NSURLQueryItem alloc]
                      initWithName:@"e"
                             value:userIdentity
                                       .identifiers[IDENTIFIER_TYPE_EMAIL]]];
  }
  ...
  if (userIdentity.identifiers[IDENTIFIER_TYPE_CUSTOM_IDENTIFIERS] != nil) {
    [queryItems
        addObject:[[NSURLQueryItem alloc]
                      initWithName:@"cstm"
                             value:[[self class]
                                       getCustomIdentifiersJson:userIdentity]]];
  }

  // Add SDK info just for analytics purposes
  [queryItems addObject:[[NSURLQueryItem alloc]
                            initWithName:@"sdkVersion"
                                   value:[ATTNAppInfo getSdkVersion]]];
  [queryItems
      addObject:[[NSURLQueryItem alloc] initWithName:@"sdkName"
                                               value:[ATTNAppInfo getSdkName]]];
...
+ (nonnull NSString *)getCustomIdentifiersJson:
  ...
  @try {
    ...
    NSData *jsonData = [NSJSONSerialization
        dataWithJSONObject:userIdentity
                               .identifiers[IDENTIFIER_TYPE_CUSTOM_IDENTIFIERS]
