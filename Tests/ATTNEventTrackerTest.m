...
- (void)testGetSharedInstance_notSetup_throws {
  // This method has multiple cases so that the order of the cases is always the
  // same. If each case was in a separate test method we could not guarantee
  // ordering.

  // **************
  // Case 1: verify that if getSharedInstance is called before setupWithSdk is
  // called then an assertion is triggered
  // **************
  ...
  [[[NSThread currentThread] threadDictionary] setValue:originalHandler
                                                 forKey:NSAssertionHandlerKey];
  // **************
  // CASE 2: verify that if setupWithSdk is called before getSharedInstance then
...
- (void)testRecordEvent_validEvent_callsApi {
  ...
  ATTNEventTracker *eventTracker =
      [[ATTNEventTracker alloc] initWithSdk:sdkMock];

  ATTNPurchaseEvent *purchase = [[ATTNPurchaseEvent alloc]
      initWithItems:[[NSArray alloc] init]
              order:[[ATTNOrder alloc] initWithOrderId:@"orderId"]];
