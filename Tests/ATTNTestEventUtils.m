...
+ (void)verifyCommonQueryItems:
  ...
  XCTAssertEqualObjects(userIdentity.identifiers[IDENTIFIER_TYPE_PHONE],
                        metadata[@"phone"]);
  XCTAssertEqualObjects(userIdentity.identifiers[IDENTIFIER_TYPE_EMAIL],
                        metadata[@"email"]);
...
+ (void)verifyProductFromItem:(ATTNItem *)item product:(NSDictionary *)product {
  ...
  XCTAssertEqualObjects(
      item.price.price,
      [[NSDecimalNumber alloc] initWithString:product[@"price"]]);
...
+ (NSDictionary *)getMetadataFromUrl:(NSURL *)url {
  NSDictionary<NSString *, NSString *> *queryItems =
      [[self class] getQueryItemsFromUrl:url];
...
+ (NSDictionary<NSString *, NSString *> *)getQueryItemsFromUrl:(NSURL *)url {
  NSArray<NSURLQueryItem *> *queryItems =
      [[NSURLComponents alloc] initWithString:url.absoluteString].queryItems;
...
+ (ATTNPurchaseEvent *)buildPurchase {
  ATTNItem *item = [[ATTNItem alloc]
      initWithProductId:@"222"
       productVariantId:@"55555"
                  price:[[ATTNPrice alloc]
                            initWithPrice:[[NSDecimalNumber alloc]
                                              initWithString:@"15.99"]
                                 currency:@"USD"]];
  ...
  ATTNPurchaseEvent *purchaseEvent =
      [[ATTNPurchaseEvent alloc] initWithItems:@[ item ] order:order];
...
+ (ATTNAddToCartEvent *)buildAddToCart {
  ...
  ATTNAddToCartEvent *event =
      [[ATTNAddToCartEvent alloc] initWithItems:@[ item ]];
  return event;
}

+ (ATTNProductViewEvent *)buildProductView {
  ATTNItem *item = [self buildItem];
  ATTNProductViewEvent *event =
      [[ATTNProductViewEvent alloc] initWithItems:@[ item ]];
  return event;
}
...
+ (ATTNPurchaseEvent *)buildPurchaseWithTwoItems {
  ...
  item1.name = @"someName";
  ATTNItem *item2 = [[ATTNItem alloc]
      initWithProductId:@"2222"
       productVariantId:@"555552"
                  price:[[ATTNPrice alloc]
                            initWithPrice:[[NSDecimalNumber alloc]
                                              initWithString:@"20.00"]
                                 currency:@"USD"]];
  ...
  cart.cartCoupon = @"someCoupon";
  ATTNPurchaseEvent *purchaseEvent =
      [[ATTNPurchaseEvent alloc] initWithItems:@[ item1, item2 ] order:order];
  purchaseEvent.cart = cart;
  return purchaseEvent;
}
...
+ (ATTNCustomEvent *)buildCustomEvent {
  return [[ATTNCustomEvent alloc] initWithType:@"Concert Viewed"
                                    properties:@{@"band" : @"The Beatles"}];
}
