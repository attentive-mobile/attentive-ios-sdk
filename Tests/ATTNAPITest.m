...
- (NSURL *)constructUserIdentityUrl:(ATTNUserIdentity *)userIdentity
...
- (id)initWithHandler:
    (void(NS_SWIFT_SENDABLE ^)(NSData *_Nullable data,
...
@implementation NSURLSessionDataTaskMock {
  void (^_completionHandler)(NSData *_Nullable data,
                             NSURLResponse *_Nullable response,
...
- (NSURLSessionDataTask *)
  ...
  if ([[url absoluteString] containsString:@"cdn.attn.tv"]) {
    _didCallDtag = true;
    return [[NSURLSessionDataTaskMock alloc]
        initWithHandler:^(NSData *data, NSURLResponse *response,
                          NSError *error) {
          completionHandler(
              [[NSString
                  stringWithFormat:@"window.__attentive_domain='%@.attn.tv'",
                                   TEST_GEO_ADJUSTED_DOMAIN]
  ...
  if ([[url absoluteString] containsString:@"events.attentivemobile.com"]) {
    ...
    return [[NSURLSessionDataTaskMock alloc]
        initWithHandler:^(NSData *data, NSURLResponse *response,
                          NSError *error) {
          completionHandler([[NSData alloc] init],
                            [[NSHTTPURLResponse alloc] initWithURL:url
                                                        statusCode:200
                                                       HTTPVersion:nil
                                                      headerFields:nil],
...
- (void)testURLSession_verifySessionHasUserAgent {
  // Arrange
  id userAgentBuilderMock =
      [OCMockObject mockForClass:[ATTNUserAgentBuilder class]];
  ...
  NSDictionary *additionalHeaders =
      [api session].configuration.HTTPAdditionalHeaders;
...
- (void)testSendUserIdentity_validIdentifiers_callsEndpoints {
  ...
  ATTNUserIdentity *userIdentity =
      [[ATTNTestEventUtils class] buildUserIdentity];
...
- (void)testSendEvent_validEvent_callsEventEndpoint {
  ...
  NSDictionary *queryItems =
      [[ATTNTestEventUtils class] getQueryItemsFromUrl:eventsUrl];
...
- (void)testSendEvent_validPurchaseEvent_urlContainsExpectedPurchaseMetadata {
  ...
  NSDictionary<NSString *, NSString *> *queryItems =
      [[ATTNTestEventUtils class] getQueryItemsFromUrl:purchaseUrl];
  NSString *queryItemsString = queryItems[@"m"];
  NSDictionary *metadata = [NSJSONSerialization
      JSONObjectWithData:[queryItemsString
                             dataUsingEncoding:NSUTF8StringEncoding]
                 options:0
                   error:nil];

  XCTAssertEqualObjects(purchase.items[0].productId, metadata[@"productId"]);
  XCTAssertEqualObjects(purchase.items[0].productVariantId,
                        metadata[@"subProductId"]);
  XCTAssertEqualObjects(
      purchase.items[0].price.price,
      [[NSDecimalNumber alloc] initWithString:metadata[@"price"]]);
  XCTAssertEqualObjects(purchase.items[0].price.currency,
                        metadata[@"currency"]);
  ...
  NSString *quantity =
      [NSString stringWithFormat:@"%d", purchase.items[0].quantity];
...
- (void)
    testSendEvent_validPurchaseEventWithTwoItems_urlContainsExpectedOrderConfirmedMetadata {
  // Arrange
  NSURLSessionMock *sessionMock = [[NSURLSessionMock alloc] init];
  ATTNAPI *api = [[ATTNAPI alloc] initWithDomain:TEST_DOMAIN
                                      urlSession:sessionMock];
  ATTNPurchaseEvent *purchase =
      [[ATTNTestEventUtils class] buildPurchaseWithTwoItems];
  ATTNUserIdentity *userIdentity =
      [[ATTNTestEventUtils class] buildUserIdentity];
  ...
  NSDictionary *ocMetadata =
      [[ATTNTestEventUtils class] getMetadataFromUrl:orderConfirmedUrl];
  NSArray<NSDictionary *> *products = [NSJSONSerialization
      JSONObjectWithData:[ocMetadata[@"products"]
                             dataUsingEncoding:NSUTF8StringEncoding]
                 options:0
                   error:nil];
  XCTAssertEqual(2, products.count);

  [[ATTNTestEventUtils class] verifyProductFromItem:purchase.items[0]
                                            product:products[0]];
  [[ATTNTestEventUtils class] verifyProductFromItem:purchase.items[1]
                                            product:products[1]];
...
- (void)testSendEvent_purchaseEventWithTwoItems_threeRequestsAreSent {
  ...
  NSDictionary *metadata =
      [[ATTNTestEventUtils class] getMetadataFromUrl:sessionMock.urlCalls[1]];
  XCTAssertEqualObjects(purchase.items[0].productId, metadata[@"productId"]);
  NSDictionary *queryItems =
      [[ATTNTestEventUtils class] getQueryItemsFromUrl:sessionMock.urlCalls[1]];
  XCTAssertEqualObjects(@"p", queryItems[@"t"]);

  // check the second item was converted to an event call
  NSDictionary *metadata2 =
      [[ATTNTestEventUtils class] getMetadataFromUrl:sessionMock.urlCalls[2]];
  XCTAssertEqualObjects(purchase.items[1].productId, metadata2[@"productId"]);
  NSDictionary *queryItems2 =
      [[ATTNTestEventUtils class] getQueryItemsFromUrl:sessionMock.urlCalls[2]];
  XCTAssertEqualObjects(@"p", queryItems2[@"t"]);

  // check an OrderConfirmed was created
  NSDictionary *metadata3 =
      [[ATTNTestEventUtils class] getMetadataFromUrl:sessionMock.urlCalls[3]];
  XCTAssertEqualObjects(purchase.order.orderId, metadata3[@"orderId"]);
  NSDictionary *queryItems3 =
      [[ATTNTestEventUtils class] getQueryItemsFromUrl:sessionMock.urlCalls[3]];
...
- (void)testSendEvent_validAddToCartEvent_urlContainsExpectedMetadata {
  ...
  NSDictionary<NSString *, NSString *> *queryItems =
      [[ATTNTestEventUtils class] getQueryItemsFromUrl:addToCartUrl];
  NSString *queryItemsString = queryItems[@"m"];
  NSDictionary *metadata = [NSJSONSerialization
      JSONObjectWithData:[queryItemsString
                             dataUsingEncoding:NSUTF8StringEncoding]
  ...
  XCTAssertEqualObjects(addToCart.items[0].productVariantId,
                        metadata[@"subProductId"]);
  XCTAssertEqualObjects(
      addToCart.items[0].price.price,
      [[NSDecimalNumber alloc] initWithString:metadata[@"price"]]);
  XCTAssertEqualObjects(addToCart.items[0].price.currency,
                        metadata[@"currency"]);
  ...
  NSString *quantity =
      [NSString stringWithFormat:@"%d", addToCart.items[0].quantity];
  XCTAssertEqualObjects(quantity, metadata[@"quantity"]);
}
...
- (void)testSendEvent_validProductViewEvent_urlContainsExpectedMetadata {
  ...
  ATTNAPI *api = [[ATTNAPI alloc] initWithDomain:TEST_DOMAIN
                                      urlSession:sessionMock];
  ATTNProductViewEvent *productView =
      [[ATTNTestEventUtils class] buildProductView];
  ATTNUserIdentity *userIdentity =
      [[ATTNTestEventUtils class] buildUserIdentity];
  ...
  NSDictionary<NSString *, NSString *> *queryItems =
      [[ATTNTestEventUtils class] getQueryItemsFromUrl:url];
  NSString *queryItemsString = queryItems[@"m"];
  NSDictionary *metadata = [NSJSONSerialization
      JSONObjectWithData:[queryItemsString
                             dataUsingEncoding:NSUTF8StringEncoding]
  ...
  XCTAssertEqualObjects(productView.items[0].productVariantId,
                        metadata[@"subProductId"]);
  XCTAssertEqualObjects(
      productView.items[0].price.price,
      [[NSDecimalNumber alloc] initWithString:metadata[@"price"]]);
  XCTAssertEqualObjects(productView.items[0].price.currency,
                        metadata[@"currency"]);
  ...
  NSString *quantity =
      [NSString stringWithFormat:@"%d", productView.items[0].quantity];
  XCTAssertEqualObjects(quantity, metadata[@"quantity"]);
}
...
- (void)
    testSendEvent_validCustomEventWithAllProperties_urlContainsExpectedCustomEventMetadata {
  // Arrange
  NSURLSessionMock *sessionMock = [[NSURLSessionMock alloc] init];
  ATTNAPI *api = [[ATTNAPI alloc] initWithDomain:TEST_DOMAIN
                                      urlSession:sessionMock];
  ...
  NSDictionary *ceMetadata =
      [[ATTNTestEventUtils class] getMetadataFromUrl:customEventUrl];
  NSDictionary *actualProperties = [NSJSONSerialization
      JSONObjectWithData:[ceMetadata[@"properties"]
                             dataUsingEncoding:NSUTF8StringEncoding]
                 options:0
                   error:nil];
...
- (void)testGetGeoAdjustedDomain_notCachedYet_savesTheCorrectDomainValue {
  ...
  [api getGeoAdjustedDomain:TEST_DOMAIN
          completionHandler:^(NSString *_Nullable geoAdjustedDomain,
                              NSError *_Nullable error) {
            XCTAssertEqualObjects(TEST_GEO_ADJUSTED_DOMAIN, geoAdjustedDomain);
          }];

  XCTAssertEqualObjects(TEST_GEO_ADJUSTED_DOMAIN,
                        [api getCachedGeoAdjustedDomain]);
}
...
- (void)testGetGeoAdjustedDomain_notCachedYet_httpMethodIsGet {
  ...
  XCTAssertEqualObjects(@"GET",
                        [sessionMock.requests[0].HTTPMethod uppercaseString]);
}
