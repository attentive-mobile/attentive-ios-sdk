...
- (void)
    testSendUserIdentity_validIdentifiers_sendsUserIdentifierCollectedEvent {
  // Arrange
  XCTestExpectation *eventTaskExpectation =
      [self expectationWithDescription:@"sendEventTask"];
  __block NSURLResponse *urlResponse;
  __block NSURL *eventUrl;

  // Act
  [api sendUserIdentity:userIdentity
               callback:^void(NSData *data, NSURL *url, NSURLResponse *response,
                              NSError *error) {
  ...
  NSDictionary<NSString *, NSString *> *queryItems =
      [[ATTNTestEventUtils class] getQueryItemsFromUrl:eventUrl];
  ...
  XCTAssertEqualObjects(
      @"[{\"vendor\":\"2\",\"id\":\"someClientUserId\"},{\"vendor\":\"1\","
      @"\"id\":\"someKlaviyoId\"},{\"vendor\":\"0\",\"id\":\"someShopifyId\"},{"
      @"\"vendor\":\"6\",\"id\":\"customIdValue\",\"name\":\"customId\"}]",
      queryItems[@"evs"]);
...
- (void)testSendEvent_validPurchaseEvent_sendsPurchaseAndOrderConfirmedEvents {
  ...
  XCTestExpectation *purchaseTaskExpectation =
      [self expectationWithDescription:@"purchaseTask"];
  __block NSURLResponse *purchaseUrlResponse;
  __block NSURL *purchaseUrl;

  XCTestExpectation *ocTaskExpectation =
      [self expectationWithDescription:@"ocTask"];
  ...
  // Purchase Event
  NSHTTPURLResponse *purchaseResponse =
      (NSHTTPURLResponse *)purchaseUrlResponse;
  XCTAssertEqual(200, [purchaseResponse statusCode]);

  NSDictionary<NSString *, NSString *> *purchaseQueryItems =
      [[ATTNTestEventUtils class] getQueryItemsFromUrl:purchaseUrl];
  NSString *purchaseQueryItemsString = purchaseQueryItems[@"m"];
  NSDictionary *purchaseMetadata = [NSJSONSerialization
      JSONObjectWithData:[purchaseQueryItemsString
                             dataUsingEncoding:NSUTF8StringEncoding]
                 options:0
                   error:nil];
  ...
  XCTAssertEqualObjects(purchase.items[0].productId,
                        purchaseMetadata[@"productId"]);
  XCTAssertEqualObjects(purchase.items[0].productVariantId,
                        purchaseMetadata[@"subProductId"]);
  XCTAssertEqualObjects(
      purchase.items[0].price.price,
      [[NSDecimalNumber alloc] initWithString:purchaseMetadata[@"price"]]);
  XCTAssertEqualObjects(purchase.items[0].price.currency,
                        purchaseMetadata[@"currency"]);
  XCTAssertEqualObjects(purchase.items[0].category,
                        purchaseMetadata[@"category"]);
  XCTAssertEqualObjects(purchase.items[0].productImage,
                        purchaseMetadata[@"image"]);
  ...
  XCTAssertEqualObjects(purchase.cart.cartCoupon,
                        purchaseMetadata[@"cartCoupon"]);

  // Order Confirmed Event
  NSHTTPURLResponse *ocResponse = (NSHTTPURLResponse *)ocUrlResponse;
  XCTAssertEqual(200, [ocResponse statusCode]);

  NSDictionary<NSString *, NSString *> *ocQueryItems =
      [[ATTNTestEventUtils class] getQueryItemsFromUrl:ocUrl];
  NSDictionary *ocMetadata =
      [[ATTNTestEventUtils class] getMetadataFromUrl:ocUrl];
...
- (void)testSendEvent_validCustomEvent_sendsCustomEvent {
  ...
  XCTAssertEqualObjects(
      ...
      [NSJSONSerialization
          JSONObjectWithData:[metadata[@"properties"]
                                 dataUsingEncoding:NSUTF8StringEncoding]
                     options:0
                       error:nil]);
}
