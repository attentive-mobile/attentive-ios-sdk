...
- (void)testIsStringAndNotEmpty_givenDictionary_returnsFalse {
  XCTAssertFalse([ATTNParameterValidation
      isStringAndNotEmpty:[[NSDictionary alloc] init]]);
}
...
- (void)testVerifyStringOrNil_givenNil_succeeds {
  XCTAssertNoThrow([ATTNParameterValidation
      verifyStringOrNil:nil
              inputName:@"inputNameValue"]);
}

- (void)testVerifyStringOrNil_givenDictionary_throws {
  XCTAssertThrows([ATTNParameterValidation
      verifyStringOrNil:@{@"hi" : @"hello"}
              inputName:@"inputNameValue"]);
}
