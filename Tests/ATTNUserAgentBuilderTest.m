...
- (void)
    testBuildUserAgent_appInfoReturnsAllValues_userAgentIsFormattedCorrectly {
  [self mockAppInfo];

  NSString *actualUserAgent = [ATTNUserAgentBuilder buildUserAgent];

  XCTAssertEqualObjects(
      @"appName-Value/appVersionValue.appBuildValue (deviceModelNameValue; "
      @"devicePlatformValue deviceOsVersionValue) sdkNameValue/sdkVersionValue",
      actualUserAgent);
}
